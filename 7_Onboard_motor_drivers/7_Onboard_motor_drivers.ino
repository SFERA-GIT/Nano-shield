/*
   ...
   Пример работы с переменными резисторами (A6,A7) и драйверами моторов левый (контакты: 3 и 12), правый (контакты: 11 и 13)
...
#define L_Motor 3       // любое упоминание сочетания символов L_Motor будет заменено компилятором на значение 3
...
//Тело функции setup
{
  pinMode(L_Motor, OUTPUT);      //настройка контакт L_Motor(3) на выход
...
//Тело функции loop
{
  ...
  if (digitalRead(FUNC_PIN) == 1) //если на входе присутвует напряжение моторы будут крутиться с заданной с помощью переменных резисторов скоростью
  {
    ...
    analogWrite(L_Motor, x);       // преобразованные значения записываем на контакт(с ШИМ) L_Motor(3)
